...
public class RawSample {
	...
	public RawSample(int[] inputDims, float[] input, int[] targetDims, float[] target){
	...
	public Sample copyInto(Sample s){
		if(s == null){
			return new Sample(new Tensor(input, inputDims), new Tensor(target, targetDims));
		} else {
			s.input.set(input);
			s.target.set(target);
			return s;
