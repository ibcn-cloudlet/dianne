 ...
 *
 */
public class NormalizedAdvantageFunctionActionStrategy implements ActionStrategy {
	...
	protected UUID stateIn, maxActionOut;
	...
	public void setup(Map<String, String> config, Environment env, NeuralNetwork... nns) throws Exception {
		...
		if (!warning.isEmpty()) {
			System.err.println(NormalizedAdvantageFunctionActionStrategy.class.getName() + ": the config variables [" + warning + "] mismatched with the action space size. Using the first value for all actions.");
		}
		
		this.theta_mu = TensorOps.cmul(this.theta_mu, this.theta, this.mu);
		
		NeuralNetworkInstanceDTO nndto = this.policy.getNeuralNetworkInstance();
		for(UUID iid : this.policy.getInputs().keySet()) {
		...
		for(UUID oid : this.policy.getOutputs().keySet()) {
			ModuleInstanceDTO mdto = nndto.modules.get(oid);
			String mname = mdto.module.properties.get("name");
			
			if(mname.equalsIgnoreCase("maxaction"))
				this.maxActionOut = oid;
		}
		if(stateIn == null || maxActionOut == null)
	...
	public Tensor processIteration(long s, long i, Tensor state) throws Exception {
		Tensor action = policy.forward(stateIn, maxActionOut, state).getValue().tensor;
