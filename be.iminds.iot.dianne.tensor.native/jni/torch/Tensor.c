...
JNIEXPORT jlong JNICALL Java_be_iminds_iot_dianne_tensor_Tensor_init
  (JNIEnv * env, jobject t, jfloatArray data, jintArray dims){
	...
	if(data != NULL){
		...
		jfloat * floats = env->GetFloatArrayElements(data, 0);
		...
		env->ReleaseFloatArrayElements(data, floats, 0);
	}
...
JNIEXPORT jintArray JNICALL Java_be_iminds_iot_dianne_tensor_Tensor_dims
  (JNIEnv * env, jobject t){
	...
	env->SetIntArrayRegion(result, 0, nDim, dims);
	return result;
}
...
JNIEXPORT jboolean JNICALL Java_be_iminds_iot_dianne_tensor_Tensor_hasDim
 (JNIEnv * env, jobject t, jintArray dims){
	...
	jsize nDim = env->GetArrayLength(dims);
...
JNIEXPORT jfloat JNICALL Java_be_iminds_iot_dianne_tensor_Tensor_get___3I
  (JNIEnv * env, jobject t, jintArray d){
	...
	jsize noDims = env->GetArrayLength(d);

	jint *index = env->GetIntArrayElements(d, 0);
	if(noDims==1){
		val = THTensor_(get1d)(
#ifdef CUDA
				state,
	...
	env->ReleaseIntArrayElements(d, index, 0);
...
JNIEXPORT jfloatArray JNICALL Java_be_iminds_iot_dianne_tensor_Tensor_get__
  (JNIEnv * env, jobject t){
	...
	real* buffer = (real*) malloc(bufferSize);
...
# endif
	...
	if(size==tensor->storage->size){
		...
		// copy right data
		narrowed = (real*) malloc(size*sizeof(real));
		real* p = narrowed;
	...
	env->SetFloatArrayRegion(result, 0, size, narrowed);
...
JNIEXPORT void JNICALL Java_be_iminds_iot_dianne_tensor_Tensor_srandn
  (JNIEnv * env, jobject t){
	...
	THTensor_(normal)(tensor, generator, 0, 1);
#endif
}
